...
func (sq *SubscriptionQuery) QueryDeliveries() *DeliveryQuery {
	query := (&DeliveryClient{config: sq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := sq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (sq *SubscriptionQuery) Clone() *SubscriptionQuery {
	...
	return &SubscriptionQuery{
		...
		withDeliveries:      sq.withDeliveries.Clone(),
		withDeadLetterTopic: sq.withDeadLetterTopic.Clone(),
...
func (sq *SubscriptionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Subscription, error) {
	var (
		...
		loadedTypes = [3]bool{
			sq.withTopic != nil,
			sq.withDeliveries != nil,
			sq.withDeadLetterTopic != nil,
		}
	)
	...
	if query := sq.withDeliveries; query != nil {
		if err := sq.loadDeliveries(ctx, query, nodes,
	...
	if query := sq.withDeadLetterTopic; query != nil {
		if err := sq.loadDeadLetterTopic(ctx, query, nodes, nil,
...
func (sq *SubscriptionQuery) loadDeliveries(ctx context.Context, query *DeliveryQuery, nodes []*Subscription, init func(*Subscription), assign func(*Subscription, *Delivery)) error {
...
func (sq *SubscriptionQuery) loadDeadLetterTopic(ctx context.Context, query *TopicQuery, nodes []*Subscription, init func(*Subscription), assign func(*Subscription, *Topic)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Subscription)
	for i := range nodes {
